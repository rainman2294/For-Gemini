...
export class ApiClient {
  ...
  constructor(config: ApiClientConfig) {
    ...
    // Initialize token from localStorage if present (so first call is authorized)
...
export class WordPressApiClient extends ApiClient {
  constructor(config?: Partial<ApiClientConfig>) {
    super({
      baseUrl: config?.baseUrl || (typeof window !== 'undefined' && (window as any).pulse2?.apiUrl) || '/wp-json/pulse2/v1',
  ...
  public async uploadMedia<T>(file: File, additionalData?: Record<string, string>): Promise<T> {
    const mediaUrl = typeof window !== 'undefined' && (window as any).pulse2?.mediaUrl 
      ? (window as any).pulse2.mediaUrl 
      : '/wp-json/wp/v2/media';
