...
export function MoodboardsView({ className, projects = [], onEnterFocusMode, onExitFocusMode }: MoodboardsViewProps) {
  ...
  const handleUpdateMoodboard = (updatedMoodboard: MoodboardWorkspace) => {
    ...
    // Persist core moodboard data into workspace settings for durability
    try {
      workspaceService.persistMoodboardState(updatedMoodboard.id, {
      ...
      console.warn('Failed to persist moodboard state:', e);
    }
  };
              ...
              return (
                ...
                >
                  ...
                  <div className="h-48 grid grid-cols-2 gap-1 bg-gray-100 overflow-hidden">
                    {(() => {
                      // Get project images (first 3) using URL extension check
                      const projectImages = (project?.media || [])
                        .filter(m => /\.(png|jpe?g|gif|webp|svg)$/i.test(m.url))
                        .slice(0, 3);
                      // Combine with moodboard images if needed
                      const moodboardImages = moodboard.images?.slice(0, 3) || [];
                      const displayImages = projectImages.length > 0 ? projectImages : moodboardImages;
                      ...
                      return displayImages.map((image, index) => (
                        <div 
                          key={index} 
                          className={`${index === 0 && displayImages.length > 1 ? 'row-span-2' : ''} relative overflow-hidden`}
                        >
                          <img 
                            src={image.url} 
