...
// Resolve JWT and API endpoints at runtime from WordPress (injected via wp_localize_script)
const getRuntimeConfig = () => {
  const g: any = (typeof window !== 'undefined' ? (window as any).pulse2 : undefined) || {};
  const apiUrl: string = g.apiUrl || '/wp-json/pulse2/v1';
  const jwtUrl: string = g.jwtUrl || '/wp-json/jwt-auth/v1/token';
  return { apiUrl, jwtUrl } as const;
};
...
export const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onLoginSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const { apiUrl, jwtUrl } = getRuntimeConfig();
      ...
      if (data.token) {
        ...
        let userId = null;
        try {
          // Query current site's API for the logged-in user
