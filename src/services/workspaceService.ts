...
const uploadFile = async (workspaceId: string, categoryId: string, file: File) => {
  ...
  const response = await fetch(`${config.apiUrl}/workspaces-json/${workspaceId}/images?category_id=${categoryId}`, {
    method: 'POST',
    headers,
    body: formData,
  });
...
class WorkspaceService {
  ...
  // Persist moodboard state (images/links) into workspace settings so it survives reloads
  async persistMoodboardState(workspaceId: string, state: { images?: Array<{ id: string; url: string; filename?: string }>; links?: Array<{ id: string; url: string; title?: string }> }): Promise<BaseWorkspace> {
    // Find current workspace to merge settings safely
    const existing = this.workspaces.find(w => w.id === workspaceId);
    const currentSettings = (existing as any)?.settings || {};
    const nextSettings = {
      ...currentSettings,
      moodboard: {
        ...(currentSettings.moodboard || {}),
        images: state.images !== undefined ? state.images : (currentSettings.moodboard?.images || []),
        links: state.links !== undefined ? state.links : (currentSettings.moodboard?.links || []),
      }
    };

    const updated = await this.updateWorkspace(workspaceId, { settings: nextSettings });
    return updated;
  }
  ...
  async getWorkspaceCategories(workspaceId: string): Promise<any[]> {
    try {
      return await apiRequest(`/workspaces-json/${workspaceId}/categories`);
    } catch (error) {
  ...
  async createWorkspaceCategory(workspaceId: string, categoryData: { name: string; color?: string }): Promise<any> {
    try {
      const category = await apiRequest(`/workspaces-json/${workspaceId}/categories`, {
        method: 'POST',
  ...
  async getWorkspaceImages(workspaceId: string): Promise<any[]> {
    try {
      return await apiRequest(`/workspaces-json/${workspaceId}/images`);
    } catch (error) {
  ...
  public async attachWorkspaceImage(workspaceId: string, categoryId: string, attachmentId: number): Promise<any> {
    try {
      return await apiRequest(`/workspaces-json/${workspaceId}/images/attach`, {
        method: 'POST',
  ...
  // Pins
  async getImagePins(imageId: string, workspaceId?: string): Promise<any[]> {
    try {
      const qs = workspaceId ? `?workspace_id=${encodeURIComponent(workspaceId)}` : '';
      return await apiRequest(`/workspaces-json/image/${imageId}/pins${qs}`);
    } catch (error) {
  ...
  async createImagePin(imageId: string, pinData: { x: number; y: number; note?: string; workspaceId: string }): Promise<any> {
    try {
      const pin = await apiRequest(`/workspaces-json/image/${imageId}/pins`, {
        method: 'POST',
  ...
  async updatePin(pinId: string, updates: { note?: string; isResolved?: boolean; workspaceId: string }): Promise<void> {
    try {
      await apiRequest(`/workspaces-json/pins/${pinId}`, {
        method: 'PUT',
  ...
  async getWorkspaceComments(workspaceId: string): Promise<any[]> {
    try {
      return await apiRequest(`/workspaces-json/${workspaceId}/comments`);
    } catch (error) {
  ...
  async createWorkspaceComment(workspaceId: string, commentData: { text: string; imageId?: string; pinId?: string }): Promise<any> {
    try {
      const comment = await apiRequest(`/workspaces-json/${workspaceId}/comments`, {
        method: 'POST',
