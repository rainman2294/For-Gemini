...
const CommentInputForm: React.FC<CommentInputFormProps> = ({ onSubmit }) => {
...
const WhiteboardDetail: React.FC<WhiteboardDetailProps> = ({ whiteboard, onBack, onPin, isPinned }) => {
  ...
  const [activeTab, setActiveTab] = useState<'pins' | 'comments'>('pins');
  ...
  // Use the useNotes hook for comment management
  const { notes } = useNotes(whiteboard.id);
  ...
  useEffect(() => {
    if (selectedImage) {
      ...
      workspaceService.getImagePins(selectedImage.id)
        .then(pins => {
  ...
  const renderPinsAndComments = () => {
    ...
    return (
      <div className="w-full">
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'pins' | 'comments')}>
          <TabsList className="grid w-full grid-cols-2 mb-4">
            <TabsTrigger value="pins" className="flex items-center gap-2"><Pin className="h-4 w-4" />Pins ({selectedImage.pins.length})</TabsTrigger>
            <TabsTrigger value="comments" className="flex items-center gap-2"><MessageCircle className="h-4 w-4" />Comments ({selectedImage.comments.length})</TabsTrigger>
          </TabsList>
          <TabsContent value="pins" className="space-y-4">
            {selectedImage.pins.length > 0 ? (
          ...
          <TabsContent value="comments" className="space-y-4">
            <CommentInputForm onSubmit={handleAddComment} />
            {selectedImage.comments.length > 0 ? (
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-start gap-3">
                      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-xs font-medium">
                        {comment.userAvatar || getUserById(comment.createdBy).name.charAt(0).toUpperCase()}
                      </div>
                      <div className="flex-1">
                        {editingComment === comment.id ? (
                          <div className="space-y-2">
                            <Textarea value={editingCommentText} onChange={(e) => setEditingCommentText(e.target.value)} className="text-sm resize-none min-h-[60px]" />
                            <div className="flex gap-2">
                              <Button size="sm" onClick={() => handleEditComment(comment.id, editingCommentText)} disabled={!editingCommentText.trim()}>Save</Button>
                              <Button size="sm" variant="outline" onClick={() => { setEditingComment(null); setEditingCommentText(''); }}>Cancel</Button>
                            </div>
                          </div>
                        ) : (
                          <>
                            ...
                            <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
                              <span>{getUserById(comment.createdBy).name}</span><span>•</span><span>{formatDate(comment.createdAt)}</span>
                            </div>
                            <div className="flex gap-1 mt-3">
                              <Button size="sm" variant="outline" onClick={() => { setEditingComment(comment.id); setEditingCommentText(comment.text); }} className="hover-shimmer cyrus-ui h-7 w-7 p-0"><Edit2 className="h-3 w-3" /></Button>
                              <Button size="sm" variant="destructive" onClick={() => handleDeleteComment(comment.id)} className="hover-shimmer cyrus-ui h-7 w-7 p-0"><Trash2 className="h-3 w-3" /></Button>
                            </div>
                          </>
                        )}
                      </div>
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>No comments yet</p>
                <p className="text-xs">Start a conversation about this image</p>
              </div>
            )}
          </TabsContent>
        </Tabs>
